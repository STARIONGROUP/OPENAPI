...
We would like to start with saying thank you for wanting to contribute to RHEA.OpenApi codebase. We want to keep it as easy as possible to contribute changes that get things working in your environment. There are a few guidelines that we need contributors to follow so that we have a chance of keeping on top of things.
...
1. Configure the upstream repo (`git remote add upstream git://github.com/RHEAGROUP/OPENAPI`) 
...
While you're working away in your branch it's quite possible that your upstream development (most likely the canonical RHEA.OpenApi version) may be updated. If this happens you should:
...
And remember; **A pull-request with tests is a pull-request that's likely to be pulled in.** :grin: Bonus points if you document your feature in our [wiki](https://github.com/RHEAGROUP/OPENAPI/wiki) once it has been pulled in

## Style Guidelines

- Indent with 4 spaces, **not** tabs.
- No underscore (`_`) prefix for member names.
- Use `this` when accessing instance members, e.g. `this.Name = "RHEA.OpenApi";`.
